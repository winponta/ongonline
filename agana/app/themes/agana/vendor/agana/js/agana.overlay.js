Agana.Overlay = function(app) {    
    ...
    this.injectLoadPageEvent = function(container) {
        ...
        var me = this;
        $('#' + this.container.id + ' #menu-container a, #' + this.container.id + ' a[load-in="'+ this.app.contentContainer.id +'"]').live('click', function(event){
            event.preventDefault(); 
            me.loadPage($(this).attr('href'));
        });

        $('#' + this.container.id + ' form[load-in="'+ this.app.contentContainer.id +'"] input[type="submit"]').live('click', function(event){
            event.preventDefault();
