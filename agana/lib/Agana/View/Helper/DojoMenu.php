 ...
 *
 * @category   Agana
 * @package    Agana_DojoMenu
 * @copyright  Copyright (c) 2011-2011 Winponta Software (http://www.winponta.com.br)
 * @author Ademir Mazer Jr (Nuno Mazer) <http://ademir.winponta.com.br>
 */
class Agana_View_Helper_DojoMenu extends Zend_View_Helper_Navigation_Menu{
...
//        }
        ...
        foreach ($container as $page) {
            ...
            if ($page->hasChildren()) {
                $id_counter++;
                $item = $this->getDojoProgrammaticMenuparts($page->getPages(), $nl . "  ", $counter_prefix . "_" . $id_counter);
                if ($item) {
                    $ret .= $nl . $item["code"];
                    $ret .= $nl . "$id.addChild(";
...
//                    $ret .= $nl . "    onClick: function () { window.location = \"" . $url . "\"; }";
                    $ret .= $nl . "    onClick: function () { loadPage('" . $url . "'); }";
                    if ($page->isActive()){
